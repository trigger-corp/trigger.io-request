...
asyncTest("Data and file", 4, function() {
	...
	forge.request.ajax({
		...
		success: function (resStr) {
			var res = JSON.parse(resStr);
			ok(res.form.a === '1', 'No array; no file: ' + JSON.stringify(res.form));
		},
		error: errorCallback
	});
	...
	forge.request.ajax({
		...
		success: function (resStr) {
			var res = JSON.parse(resStr);
			ok(res.form['a[0]'] === '1', 'With array; no file: ' + JSON.stringify(res.form));
		},
		error: errorCallback
	});
});